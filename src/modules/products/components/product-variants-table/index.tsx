  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="overflow-x-auto p-px">
        <div className="border-2 border-abc-gold rounded-xl overflow-hidden shadow-lg">
          <Table className="w-full">
            <Table.Header className="border-t-0">
              <Table.Row className="bg-abc-accent border-none">
                {product.options?.map((option) => {
                  if (option.title === "Default option") {
                    return null
                  }
                  return (
                    <Table.HeaderCell key={option.id} className="px-4 border-x text-abc-red font-merriweather font-semibold">
                      {option.title}
                    </Table.HeaderCell>
                  )
                })}
                <Table.HeaderCell className="px-4 border-x text-abc-red font-merriweather font-semibold">
                  Price
                </Table.HeaderCell>
                <Table.HeaderCell className="px-4 text-abc-red font-merriweather font-semibold">Quantity</Table.HeaderCell>
              </Table.Row>
            </Table.Header>
                ...
                return (
                  ...
                  >
                    {variant.options?.map((option, index) => {
                      ...
                      return (
                        <Table.Cell key={option.id} className="px-4 border-x text-abc-red font-merriweather">
                          {option.value}
                        </Table.Cell>
                      )
                    })}
                    <Table.Cell className="px-4 border-x text-abc-red font-merriweather">
      ...
      <Button
        ...
        variant="primary"
        className="w-full h-10 hover:bg-abc-accent hover:text-abc-red font-merriweather"
        isLoading={isAdding}
